...
contract TokenAuctionWithHeap is ReentrancyGuard {
    ...
    Heap.Uint256Heap private orderedBidHeap;
    ...
    function placeBid(uint256 _quantity, uint256 _bidPricePerUnit) external payable auctionActive nonReentrant {
        ...
        if(bidderCount != 0) {
            require(_bidPricePerUnit > orderedBidHeap.peek(), "Bid too low");
        }

        bytes32 bidHash = _createBid(_quantity, _bidPricePerUnit);
        
        orderedBidHeap.insert(_bidPricePerUnit);
        bidderCount++;
    ...
    function _fillWinningBids() internal {
        ...
        while (remainingTokens > 0 && orderedBidHeap.length() > 0) {
            uint256 highestBidPerValue = orderedBidHeap.pop();
